...
export function SectionColumn({
  ...
  return (
    <div
      className="sticky left-0 z-20 flex h-full min-h-full flex-col justify-between gap-4 border-r border-[hsl(var(--border))]/70 bg-[rgba(248,245,237,0.95)] px-4 py-4 backdrop-blur-xl"
      style={{ height: rowHeight }}
    >
      <div className="min-w-0">
        <div className="text-xs font-semibold uppercase tracking-[0.24em] text-slate-400">Section</div>
        <div className="mt-2 truncate text-base font-semibold tracking-[-0.02em] text-[hsl(var(--foreground))]">
          {section.title}
        </div>
      </div>
      <div className="flex max-w-[190px] flex-wrap gap-2">
        ...
        <Button
          variant="ghost"
          className="h-9 w-9 px-0 text-rose-700 hover:border-rose-200/70 hover:bg-rose-50/85"
        ...
        {!isCompact ? <div className="basis-full pt-1 text-[11px] font-medium uppercase tracking-[0.18em] text-slate-400">Edit row actions</div> : null}
      </div>
    </div>
  );
}
